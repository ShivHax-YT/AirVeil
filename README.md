...
**Face light** is AirVeil’s own screen-edge illumination, off at the start of every check. The notch offers a central On/Off button only after repeated observations of a close face whose pose is unreadable in low face-region light. An empty dark frame, a distant face, or a measurable off-center pose does not trigger the card. Turning it on returns to the camera view; a control remains available to turn it off. Success, cancellation, stale/missing face evidence, and inactivity turn it off. It never changes the display’s brightness setting or Apple’s independent Edge Light preference. See the [Face light flow](research/FACE-LIGHT-FLOW.md).
...
- **Automatically manage displays when AirPods are removed** is an optional, separate control that works even while blur is paused. After live headphone motion has been received, either a headphone disconnect or validated loss of an in-ear bud starts the removal policy after a 1.5-second reconnect delay. Keep Automatic Ear Detection enabled. Per-bud metadata supplements public connection events when supported, so removing the nonstreaming bud can also trigger the check. A known per-bud removal persists across transport reconnects until a stable worn-bud gain or explicit manual recovery. Motion gaps and calibration jumps alone do not trigger removal.
...
Per-bud wear status uses runtime-checked, read-only private IOBluetooth getters on a utility queue. A unique connected compatible AirPods device and stable evidence are required; unsupported or stale metadata falls back to public connection events without inventing per-bud removal. No scanning, pairing, connection changes, identifiers, or wear history are saved. Bluetooth permission may be requested by macOS. See the [per-bud and zero-brightness review](research/REMOVAL-ZERO-REVIEW.md).
