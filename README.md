...
**Source version: 0.15.0, build 30.** Build 30 is installed and verified locally, with full regression passing. It adds foreground-region quality checks and restores brightness before accepting departure after losing the seat while dimmed. This follows two wearer-confirmed false locks in build 29; the physical retest is pending. Independent dim/lock switches, dismiss-only reminder, and brightness guidance remain available. See the [current goal checklist](validation/GOAL-CHECKLIST-0.15.0-30.md) and [brightness test record](validation/SETTINGS-WEAR-AND-WAKE-0.15.0-24.md). The download above remains published **0.14.0 RC 1**; build 30 is not a published release.
