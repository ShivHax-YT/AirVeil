...
Hover near the notch to reveal **Set center**, **Enable/Pause**, and settings. **Show Notch Controls** in the AirVeil menu is an alternative to hovering. With camera assistance enabled, automatic wake/rewear checks and manual Set center show a circular mirrored preview beneath the hardware cutout. The curved indicator follows head rotation from fresh AirPods samples between camera observations. A matched, previously learned camera reference anchors its displayed angle; before that reference is available, it explicitly shows relative head movement. Missing motion hides the marker instead of pretending the wearer faces center. Measured dim frames with failed face detection produce a specific lighting message. No-face, multiple-face, motion, and camera errors have separate guidance.

The coach shows confirmation only when the existing camera/AirPods coordinator has restored a valid heading. It then dismisses after a short settle. Progress reflects accepted paired samples; it is not an arbitrary animation timer. Thumbnail framing is guidance and does not add calibration restrictions. Camera samples wait for actual AirPods evidence on both sides of capture time. A first setup includes a prompted head turn; a later deliberate Set center reuses the learned direction for the same camera configuration, including after a display-layout change. Recovery still measures the current angle against the saved reference, without replacing zero. The live indicator is feedback only and cannot approve calibration or drive the desktop effect. Camera images are released on stop, cancel, timeout, and sleep.
