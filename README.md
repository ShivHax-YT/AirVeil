...
**Source version: 0.15.0, build 29.** Build 29 is installed and verified locally, with the full regression passing. It repairs stale internal lock state after delayed notifications while retaining brightness recovery before cameras resume. Build 28 added independent dim/lock switches, a dismiss-only wear reminder, and low-light guidance with continued seat monitoring. New physical acceptance is pending. Build 27 restored original brightness in four wearer-confirmed cases: both return orders after manual lock and Apple-menu Sleep. The download above remains the published **0.14.0 RC 1**. See the [current goal checklist](validation/GOAL-CHECKLIST-0.15.0-29.md) and [brightness test record](validation/SETTINGS-WEAR-AND-WAKE-0.15.0-24.md). Build 29 has not been published as a release.
