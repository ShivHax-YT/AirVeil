...
**Source version: 0.15.0, build 28.** Build 28 is installed and verified locally, with the full regression and native UI checks passing. It adds independent dim/lock switches, a dismiss-only wear reminder, and a low-light explanation followed by brightness restoration and continued seat monitoring. New physical acceptance is pending. Build 27 restored original brightness in four wearer-confirmed cases: both AirPods return orders after manual lock and Apple-menu Sleep. The download above still points to the published **0.14.0 RC 1**; these changes require the current 0.15.0 build until its release is published. See the [goal checklist](validation/GOAL-CHECKLIST-0.15.0-28.md) and [brightness test record](validation/SETTINGS-WEAR-AND-WAKE-0.15.0-24.md). Build 28 has not been published as a release.
