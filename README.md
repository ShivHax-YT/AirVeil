...
**Source version: 0.15.0, build 25.** This README describes the current working source. The download link above still points to the published **0.14.0 RC 1**; the new permission cards and behavior below require a 0.15.0 source build until its release is published. The [current validation record](validation/SETTINGS-WEAR-AND-WAKE-0.15.0-24.md) separates build 24's installed evidence, pending physical acceptance, and build 25's status-wording follow-up.
...
**Sync head**, beneath the onset illustration in Appearance, requests a real camera and AirPods alignment. Face the camera and hold still for the check. After it succeeds, the illustration follows your left and right turns independently of the selected onset side and **Invert direction**. Sync does not change onset thresholds. It does not start desktop blur by itself; if blur was already active, it can resume after valid alignment. **Stop sync**, leaving Appearance, closing Settings, or minimizing Settings stops the live illustration. If Head Tracking access is missing, review Permissions and press Sync head again.
...
The permission stage uses white glass cards over a quiet field of small white stars that fade independently on and off, with short meteor streaks every 4.5 seconds. macOS 26 or later uses native Liquid Glass; older systems use a translucent material. Side cards stay blurred and dimmed. Reduce Motion keeps the background still and removes card movement, while Reduce Transparency gives the cards a solid readable surface.
...
- **Automatically manage displays when both AirPods are removed** is optional and works even while blur is paused, unless **Turn off feature** has paused automatic checks. A stable session of advancing headphone-motion samples arms the workflow. Sustained loss of the entire motion stream then starts a separate 1.5-second return delay before the display action. Keep Automatic Ear Detection enabled. The public motion signal cannot identify individual earbuds or prove both are out: a sustained connection interruption can produce the same signal. Removing one bud while the other keeps supplying motion, a continuous left/right source handoff, or a brief interruption does not trigger this workflow. A single returning packet can restore owned brightness, but a new stable motion session is needed to arm another loss.
- **Dim while I am still seated**, together with camera assistance, uses the most recent successful center check to remember foreground-seat geometry for this app session. The built-in camera checks whether that seat remains occupied. A foreground body can remain present while turned away; smaller background or side occupants are excluded. Confirmed presence dims the built-in panel to the adjustable 0–50% target (0% default, black without locking) and prevents idle display sleep. Fresh motion returning stops presence capture and restores the original brightness before camera heading recovery; motion may return with only one AirPod in an ear. External displays are not dimmed.
- In the camera-presence mode, confirmed departure keeps an already dimmed panel at its current level through the display-sleep request, avoiding a flash before sleep. Ambiguity, darkness, or missing camera evidence receives an 8-second grace period, then the check ends and owned brightness is restored. An unavailable seat reference also ends the check without display sleep. Use **Set center** after launching or moving the Mac to refresh the seat. Confirmed departure is latched for that removal episode. Geometry cannot distinguish somebody silently replacing the wearer in the same seat; it is not identity verification.
- If automatic display management stays enabled while camera assistance or seated dimming is off, sustained motion loss requests display sleep directly, without a seat check. Turning off the camera alone does not turn off automatic display management; disable that separate control if you do not want automatic sleep.
- Brightness restoration retains the original level through slider changes, and a small persistent journal recovers interrupted writes. Readings from an asleep display cannot replace the saved baseline. If a new lock or sleep interrupts restoration, the journal remains pending until an awake recovery verifies it. Heading recovery waits for brightness cleanup whether AirPods return before or after unlock. An awake manual brightness adjustment still takes precedence. Pause, disabling the feature, and normal quit restore owned brightness; asleep or inactive sessions defer restoration until wake/unlock. Both return orders have automated regression coverage; repeated physical acceptance is still pending for build 24.
...
- **Reset defaults** restores effect settings, selects all connected displays, disables camera assistance and automatic display off, and pauses the effect. It does not revoke macOS permissions or erase saved camera setup, tutorial completion, or permission-setup records.
- **Start blur when turning** has independent Left and Right controls, each from 0–60°. Drag the semicircular dial; its unsynced illustration demonstrates the selected side. **Sync head** makes the head follow live camera-aligned AirPods motion without changing either threshold. Keyboard/accessibility adjustments use one-degree steps. The default is 8° on each side; full effect is 32°. Increasing onset beyond the full-effect threshold raises that threshold to preserve a usable transition.
...
- Display sleep is requested once per motion-loss episode. Fresh motion returning during the delay, pressing Pause, switching this setting off, or quitting cancels a pending request. Waking without a new wearing session does not immediately turn the displays off again. Reset defaults disables this option.
