...
**Source version: 0.15.0, build 23.** This README describes the current working source. The download link above still points to the published **0.14.0 RC 1**; the new permission cards and behavior below require a 0.15.0 source build until its release is published.
...
- Optional camera assistance needs the Mac’s built-in camera and **Camera** access. It runs only after you enable it during permission setup or in Settings.
...
Camera assistance is optional. Choose it during permission setup or use **Enable camera assistance** in Settings to request Camera access. A new install keeps it off if you continue without camera access. The app uses only the built-in Mac camera, processes images locally in memory, and does not select an iPhone or external camera automatically.
