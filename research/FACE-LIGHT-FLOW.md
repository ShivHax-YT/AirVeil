...
The wearer selected AirVeil's own light with a direct On/Off button for this update. This is a screen border rendered by AirVeil, not an API toggle for Apple's Edge Light. It begins off each check, uses only the built-in display, and changes neither display backlight brightness nor a persistent preference.
...
3. Offer the light card only after two consecutive frames show one plausible, nearby face but cannot produce a reliable frontal pose, with face-local luminance below 0.18. This threshold is a heuristic; no photometric or physical angle accuracy guarantee is claimed.
4. Missing faces, multiple faces, tiny distant faces, stale/untimed frames, and clearly measurable off-axis turns do not offer the light. A confident usable frontal face never needs the card merely because its pixels are dark.
5. The card's explicit On action illuminates the border and returns to the same capture. It clears pre-light hold evidence; it does not restart the camera, extend its existing deadline, save a new center, or grant alignment.
...
- Camera coordinator: 109 checks, including empty-room darkness, off-axis darkness, two-frame light offer, explicit toggle, one capture/unchanged center, light cleanup, same-epoch recovery and bounded retry.
