...
## Press and drag tracking — build 35

User testing still found no physical feedback in build 34, especially while the native glass indicator was dragged between tabs. Selection-callback tests alone did not cover this interaction. Settings now installs a window-scoped AppKit observer that recognizes only the five labeled native tabs. It requests feedback on mouse-down (including the already-selected tab), samples boundary crossings only during the held press in the native event-tracking run-loop mode, and forwards every event unchanged. The timer stops on release, deactivation, detachment, or hidden window; ordinary hover is silent. A 45 ms interval bounds ticks, and the later committed selection is suppressed to avoid duplicates. The shared preference still controls every pulse.

Native regression now also checks actual toolbar hit testing, initial press routing, boundary geometry, duplicate samples, and observer cleanup with an injected performer. These are software checks, not proof of physical sensation. The previously reported physical failure is not considered user-verified until tested on the trackpad again.
