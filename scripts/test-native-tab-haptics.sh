...
swiftc -sdk "$TASK_SDK" -target "$(uname -m)-apple-macos14.0" -swift-version 6 \
  "$TASK_ROOT/Sources/InteractionHaptics.swift" "$TASK_ROOT/Tests/NativeTabHapticsTests.swift" \
  -o "$TASK_ROOT/build/tests/native-tab-haptics"
"$TASK_ROOT/build/tests/native-tab-haptics"
